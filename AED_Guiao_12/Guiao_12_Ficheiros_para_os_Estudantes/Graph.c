...
// TO BE COMPLETED !!
  ...
  copy->numEdges = 0; /* will set after copying edges */

  copy->verticesList = ListCreate(graphVerticesComparator);
  if (copy->verticesList == NULL) abort();

  /* Iterate original vertices and copy each vertex and its edge list */
  List* vertices = g->verticesList;
  if (ListIsEmpty(vertices) == 0) {
    ListMoveToHead(vertices);
    for (unsigned int i = 0; i < g->numVertices; ListMoveToNext(vertices), i++) {
      ...
      struct _Vertex* vcopy = (struct _Vertex*)malloc(sizeof(struct _Vertex));
      if (vcopy == NULL) abort();

      vcopy->id = v->id;
      vcopy->inDegree = v->inDegree;
      vcopy->outDegree = v->outDegree;
      vcopy->edgesList = ListCreate(graphEdgesComparator);
      if (vcopy->edgesList == NULL) abort();

      /* Copy edges of this vertex */
      List* edges = v->edgesList;
      if (ListIsEmpty(edges) == 0) {
        ListMoveToHead(edges);
        for (int j = 0; j < ListGetSize(edges); ListMoveToNext(edges), j++) {
          struct _Edge* e = ListGetCurrentItem(edges);
          struct _Edge* ecopy = (struct _Edge*)malloc(sizeof(struct _Edge));
          if (ecopy == NULL) abort();
          ecopy->adjVertex = e->adjVertex;
          ecopy->weight = e->weight;
          if (ListInsert(vcopy->edgesList, ecopy) == -1) {
            free(ecopy);
            abort();
          }
        }
      ...
      /* Insert the copied vertex into the new graph's vertices list */
      if (ListInsert(copy->verticesList, vcopy) == -1) {
        /* cleanup would be needed in production code; abort for simplicity */
        abort();
      }
    }
  }

  /* Preserve the number of edges from the original graph */
